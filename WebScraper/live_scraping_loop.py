...
def main():
    #schedule.every(30).minutes.do(append_time)
    append_time()
...
def control_live_scraping_loop():
    schedule.every(15).seconds.do(live_scraping_loop)
    while True:
        schedule.run_pending()
        time.sleep(1)
...
def count_time():
    live_scraping_loop()
    while (ongoing_match == True):
        control_live_scraping_loop()
    for i in range(10):
        time.sleep(30)
...
def check_time():
    if (len(live_match_time) != 0):
        for a in live_match_time:
            if (a < time.time() - 60 * 30):
                live_match_time.remove(a)
            if (((a - 60 * 15) < time.time()) and ((a + 60 * 15) > time.time())):
                count_time()
